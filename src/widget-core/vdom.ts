...
import { Registry, isWidgetBaseConstructor, isWidget } from './Registry';
...
export interface UseFunction {
	<T extends MetaBase>(MetaType: WidgetMetaConstructor<T>): T;
	__setProperties__: (properties: any) => void;
	__destroy__: () => void;
}
...
function createUseController(invalidator: any, nodeHandler: any, registry: any, properties: any): UseFunction {
	const _metaMap = new Map<WidgetMetaConstructor<any>, MetaBase>();
	const use: any = <T extends MetaBase>(MetaType: WidgetMetaConstructor<T>): T => {
		let cached = _metaMap.get(MetaType);
		if (!cached) {
			cached = new MetaType({
			...
			_metaMap.set(MetaType, cached);
		}

		return cached as T;
	};
	use.__setProperties__ = (properties: any) => {
		_metaMap.forEach((value) => {
			value.__setProperties__ && value.__setProperties__(properties);
		});
	};
	use.__destroy__ = function() {
		_metaMap.forEach((value) => {
			value.destroy();
		});
	};
...
export function renderer(renderer: () => WNode | VNode): Renderer {
	...
	let _idToWrapperMap = new Map<number, WNodeWrapper>();
	let _widgetMetaMap = new Map<number, WidgetMeta>();
	...
	function resolveRegistryItem(wrapper: WNodeWrapper, instance?: WidgetBase) {
		...
		if (instance) {
			...
			if (instanceData && !isWidget(wrapper.node.widgetConstructor)) {
	...
	function findInsertBefore(next: DNodeWrapper) {
		...
		while (!insertBefore) {
			...
			if (!searchNode || isVNodeWrapper(searchNode || !someSillyMap.get(next)!.requiresSearch)) {
				break;
			}
		}
		return insertBefore;
	...
	const someSillyMap = new WeakMap();
	...
	function _createWidget({ next }: CreateWidgetInstruction): ProcessResult | false {
		...
		if (!isWidgetBaseConstructor(Constructor)) {
			const invalidator = () => {
				const widgetMeta = _widgetMetaMap.get(next.id);
				if (widgetMeta) {
					widgetMeta.dirty = true;
				}
				_invalidationQueue.push({ id: next.id, depth: next.depth, order: next.order });
				_schedule();
			};
			...
			const widgetMeta: WidgetMeta = {
				...
				registry,
				nodeHandler,
				use: createUseController(invalidator, nodeHandler, registry, next.properties)
			};
			_widgetMetaMap.set(next.id, widgetMeta);
			...
			instance = new Constructor() as WidgetBase;
			if (registry) {
	...
	function _updateWidget({ current, next }: UpdateWidgetInstruction): ProcessResult {
		...
		if (!isWidgetBaseConstructor(Constructor)) {
			const widgetMeta = _widgetMetaMap.get(next.id);
			if (widgetMeta && !Constructor.isStatic) {
				widgetMeta.use.__setProperties__(next.node.properties);
	...
	function _removeWidget({ current }: RemoveWidgetInstruction): ProcessResult {
		...
		const meta = _widgetMetaMap.get(current.id);
		if (meta) {
			meta.nodeHandler.destroy();
			meta.registry.destroy();
			meta.use.__destroy__();
			meta.invalidator = undefined;
			_widgetMetaMap.delete(current.id);
		}
				...
				while ((wrapper = wrappers.pop())) {
					if (isWNodeWrapper(wrapper)) {
						...
						if (wrapper.instance) {
							...
							const meta = _widgetMetaMap.get(wrapper.id);
							if (meta) {
								meta.nodeHandler.destroy();
								meta.registry.destroy();
								meta.use.__destroy__();
								_widgetMetaMap.delete(wrapper.id);
							}
						}
					}
