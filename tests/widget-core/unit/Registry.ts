...
import { ESMDefaultWidgetBase, Constructor } from '../../../src/widget-core/interfaces';
	...
	Widget: {
		...
		get: {
			...
			'get allows a generic to passed that defines the type of registry item'() {
				...
				const RegistryTestWidget = factoryRegistry.get<TestWidget>('test-widget') as Constructor<TestWidget>;
		...
		emit: {
			...
			'emits loaded event when defining a function'() {
				let loadedEvent = false;
				const registry = new Registry();
				registry.on('foo', ({ type }) => {
					loadedEvent = true;
