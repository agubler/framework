...
import { VNode, DNode, DomVNode, RenderOptions } from '../../../src/widget-core/interfaces';
...
jsdomDescribe('vdom', () => {
	...
	describe('functional widgets', () => {
		...
		it('should create elements for widgets', () => {
			...
			function MainFoo({ properties }: RenderOptions<any>) {
			...
			function TestWidget({ properties }: RenderOptions<any>) {
				return v('span', { classes: ['myClass'] }, [w(MainFoo, { show: properties.show })]);
			}
		...
		it('Should re-render widgets on property diffs', () => {
			let renderCount = 0;
			function Foo({ properties }: RenderOptions<any>) {
				renderCount++;
			...
			class App extends WidgetBase {
				...
				render() {
					return v('div', [this._text === 'hide' ? null : w(Foo, { text: this._text })]);
				}
			}
			...
			assert.strictEqual(div.outerHTML, '<app><div><div>first</div></div></app>');
			assert.strictEqual(renderCount, 1);
			updateText('second');
			assert.strictEqual(div.outerHTML, '<app><div><div>second</div></div></app>');
			assert.strictEqual(renderCount, 2);
			updateText('second');
			assert.strictEqual(div.outerHTML, '<app><div><div>second</div></div></app>');
			assert.strictEqual(renderCount, 2);
			updateText('hide');
			assert.strictEqual(div.outerHTML, '<app><div></div></app>');
			assert.strictEqual(renderCount, 2);
		});
		...
		it('supports widget registry items', () => {
			...
			function Foo({ properties }: RenderOptions<any>) {
				return v('h1', [properties.text]);
			}
			function Bar({ properties }: RenderOptions<any>) {
				return v('h2', [properties.text]);
			}

			registry.define('foo', () => Foo);
			registry.define('bar', () => Bar);
			function Baz() {
				return v('div', [w<{ text: string }>('foo', { text: 'foo' }), w<any>('bar', { text: 'bar' })]);
			}
		...
		it('should invalidate when a registry items is loaded', () => {
			...
			function Baz() {
				return v('div', [w('foo', { text: 'foo' }), w('bar', { text: 'bar' })]);
			}
		...
		it('scopes registry to the widget that the WNode is defined', () => {
			function Foo({ children }: RenderOptions) {
				return children;
			}
			...
			function Qux({ registry }: RenderOptions) {
				if (!registry.has('bar')) {
					registry.define('bar', () => Bar);
				}
		...
		it('supports an array of DNodes', () => {
			...
			function Foo() {
				myClass = !myClass;
				const classes = myClass ? ['myClass'] : [];

				return [v('div', { classes }, ['1']), v('div', {}, ['2']), v('div', { classes: ['myClass'] }, ['3'])];
			}
	...
	describe('class widgets', () => {
		...
		it('supports widget registry items', () => {
			...
			class Baz extends WidgetBase {
				render() {
					return v('div', [w<any>('foo', { text: 'foo' }), w<any>('bar', { text: 'bar' })]);
				}
			}
